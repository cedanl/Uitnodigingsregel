...
try:
    # --- 1. Train/test split ---
    # Single split on unscaled data, then use same indices for scaled data
    # This ensures RF, Lasso, and SVM are all evaluated on the exact same students
    ...
    # Use the same split indices to slice the scaled data
    # train_df_sdd has a reset index (0-based) from standardize_dataset,
    # so we need positional indexing to match the same rows
...
fig = generate_precision_plot(test_data_rf, best_rf_model, best_lasso_model, best_svm_model, validation_data_scaled=test_data_sdd, do_save=False)
...
# Generate the plot using test set data for consistency with evaluation
# Use test_data_rf for RF, test_data_sdd for Lasso and SVM
fig = generate_sensitivity_plot(test_data_rf, best_rf_model, best_lasso_model, best_svm_model, validation_data_scaled=test_data_sdd, do_save=False)
plt.tight_layout(); plt.show()
```
