...
from uitnodigingsregel.analyze import (
    analyze_missing_data,
    display_top_features,
    parse_model_metrics,
)
from uitnodigingsregel.dataset import detect_separator, impute_missing_values, remove_single_value_columns
from uitnodigingsregel.evaluate import (
...
from uitnodigingsregel.visualize import (
...
settings = load_settings(settings_type='default')
retrain_models = settings['retrain_models']
dropout_column = settings['dropout_column']
random_seed = settings['random_seed']
rf_parameters = settings['rf_parameters']
alpha_range = settings['alpha_range']
svm_parameters = settings['svm_parameters']
...
user_train_path = Path(settings['user_data_dir_train'])
if user_train_path.exists():
    train_sep = detect_separator(user_train_path, dropout_column)
    train_df = pd.read_csv(user_train_path, sep=train_sep, engine='python')
    data_type = "instelling"
else:
    synth_train_path = Path(settings['synth_data_dir_train'])
    train_sep = detect_separator(synth_train_path, dropout_column)
    train_df = pd.read_csv(synth_train_path, sep=train_sep, engine='python')
    data_type = "synthetische"

train_cleaned = train_df.drop_duplicates()
pred_cleaned = train_cleaned.copy()
train_cleaned, pred_cleaned = impute_missing_values(train_cleaned, pred_cleaned)
train_cleaned, pred_cleaned = remove_single_value_columns(train_cleaned, pred_cleaned)
train_processed, pred_processed = convert_categorical_to_dummies(train_cleaned, pred_cleaned, dropout_column)
train_df_sdd, pred_df_sdd = standardize_dataset(train_processed, pred_processed, dropout_column)
...
try:
    ...
    train_positions = train_processed.index.get_indexer(X_train.index)
    test_positions = train_processed.index.get_indexer(X_test.index)
    X_train_sdd = train_df_sdd.drop(dropout_column, axis=1).iloc[train_positions].reset_index(drop=True)
    X_test_sdd = train_df_sdd.drop(dropout_column, axis=1).iloc[test_positions].reset_index(drop=True)
    y_train_sdd = train_df_sdd[dropout_column].iloc[train_positions].reset_index(drop=True)
    y_test_sdd = train_df_sdd[dropout_column].iloc[test_positions].reset_index(drop=True)
    ...
    if retrain_models:
        with redirect_stdout(io.StringIO()), redirect_stderr(io.StringIO()):
                best_rf_model = train_random_forest(pd.concat([X_train, y_train], axis=1), random_seed, dropout_column, rf_parameters)
                best_lasso_model = train_lasso(pd.concat([X_train_sdd, y_train_sdd], axis=1), random_seed, dropout_column, alpha_range)
                best_svm_model = train_svm(pd.concat([X_train_sdd, y_train_sdd], axis=1), random_seed, dropout_column, svm_parameters)
    else:
    ...
    # --- 3. Stoplight evaluation on test set ---
    model_predictions = {
...
fig = generate_precision_plot(test_data_rf, best_rf_model, best_lasso_model, best_svm_model, validation_data_scaled=test_data_sdd, dropout_column=dropout_column, do_save=False)
...
fig = generate_sensitivity_plot(test_data_rf, best_rf_model, best_lasso_model, best_svm_model, validation_data_scaled=test_data_sdd, dropout_column=dropout_column, do_save=False)
plt.tight_layout(); plt.show()
```
...
print(display_top_features(best_svm_model, train_df_sdd, 'svm', n_features=5, dropout_column=dropout_column))
```
...
fig = generate_svm_importance_plot(train_processed, best_svm_model, train_data_sdd=train_df_sdd, dropout_column=dropout_column, do_save=False)
```
